...
public class FeedbackServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String queryType = (String) hs.getAttribute("queryType");
        try {
            ...
            String queryInsert = "insert into feedback(querytype, email, name, mobile, city, state, comment) values('" + queryType + "', '" + email + "', '" + name + "', '" + contact + "', '" + city + "', '" + state + "', '" + comment + "')";
            st1.executeUpdate(queryInsert);
            hs.setAttribute("username", username);
            if (queryType.equals("car")) {
                response.sendRedirect("Welcome.jsp");
            }
            else if(queryType.equals("mobile")){
                response.sendRedirect("WelcomeMobile.jsp");
            }
            else if(queryType.equals("laptop")){
